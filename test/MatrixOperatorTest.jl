using ApproxFunBase, ApproxFunOrthogonalPolynomials, ApproxFunSingularities
...
@testset "MatrixOperator" begin
    ...
    @test C[1,1] == ApproxFunBase.ZeroOperator(Legendre(),Chebyshev())
    @test C[1,2] == Conversion(Chebyshev(),Chebyshev())
    @test C[2,1] == Conversion(Legendre(),Legendre())
    @test C[2,2] == ApproxFunBase.ZeroOperator(Chebyshev(),Legendre())
    ...
    plan=[3,1,2]
    C=Conversion(A,B,plan)
    @test C[1,1] == ApproxFunBase.ZeroOperator(JacobiWeight(1,0,Jacobi(1,-1)),JacobiWeight(2/3,0,Jacobi(2/3,-2/3)))
