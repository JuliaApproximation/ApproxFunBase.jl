...
for TYP in (:SumSpace,:PiecewiseSpace)
    @eval function Conversion(@nospecialize(S1::$TYP), @nospecialize(S2::$TYP))
...
for (OPrule,OP) in ((:conversion_rule,:conversion_type),(:maxspace_rule,:maxspace),
                        (:union_rule,:union))
    for TYP in (:SumSpace,:PiecewiseSpace)
        @eval function $OPrule(@nospecialize(S1sp::$TYP), @nospecialize(S2sp::$TYP))
            S1 = components(S1sp)
            S2 = components(S2sp)
...
function Derivative(@nospecialize(S::SumSpace), k::Integer)
